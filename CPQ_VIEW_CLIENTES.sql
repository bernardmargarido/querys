...
/****** Object:  View [dbo].[cpq_clientes]    Script Date: 08/02/2023 16:32:38 ******/
SET ANSI_NULLS ON
GO
...
(
	...
	SELECT
		...
		,ISNULL(
			...
			FROM [LABOR-1233]..SE1040 SE1 (NOLOCK) 
			WHERE
	...
	FROM [LABOR-1233]..SA1040 SA1 (NOLOCK)
	LEFT JOIN [LABOR-1233]..ACY040 ACY (NOLOCK)
		ON ACY.ACY_FILIAL = ' '
		AND ACY.ACY_GRPVEN = SA1.A1_GRPVEN
		AND ACY.D_E_L_E_T_ = ' '
	LEFT JOIN [LABOR-1233]..XTI040 XTI (NOLOCK)
		ON XTI.XTI_FILIAL = ' '
		AND XTI.XTI_CODIGO = SA1.A1_XSGMCRE
		AND XTI.D_E_L_E_T_ = ' '
	LEFT JOIN [LABOR-1233]..SA3040 SA3 (NOLOCK)
		ON SA3.A3_FILIAL = ' '
		AND SA3.A3_COD = SA1.A1_VEND
		AND SA3.D_E_L_E_T_ = ' '
	LEFT JOIN [LABOR-1233]..SA3040 SA3_2 (NOLOCK)
		ON SA3_2.A3_FILIAL = ' '
		AND SA3_2.A3_COD = SA1.A1_XVEND2
		AND SA3_2.D_E_L_E_T_ = ' '
	LEFT JOIN [LABOR-1233]..CC3040 CC3 (NOLOCK)
		ON CC3.CC3_FILIAL = ' '
		AND CC3.CC3_COD = SA1.A1_CNAE
		AND CC3.D_E_L_E_T_ = ' '
	LEFT JOIN [LABOR-1233]..SZ5040 SZ5 (NOLOCK)
		ON SZ5.Z5_FILIAL = ' '
	...
	FROM [LABOR-1233]..SUS040 SUS (NOLOCK)
	LEFT JOIN [LABOR-1233]..SA3040 SA3 (NOLOCK)
		ON SA3.A3_FILIAL = ' '
